...
public class OrderUseCase implements OrderServicePort {
    ...
    public OrderModel createOrder(OrderModel orderModel) {
        ...
        if (orderPersistencePort.hasActiveOrdersForCustomer(customerId)) {
            throw new CustomOrderException(DomainMessagesConstants.ORDER_CUSTOMER_HAS_ACTIVE_ORDER);
        }

        Optional<RestaurantModel> restaurant = restaurantPersistencePort.findRestaurantById(orderModel.getRestaurant().getId());
        ...
        for (OrderDishModel orderDish : orderModel.getOrderDishes()) {
            ...
            if (!dish.get().getRestaurant().getId().equals(orderModel.getRestaurant().getId())) {
                throw new CustomOrderException(DomainMessagesConstants.ORDER_DISH_DIFFERENT_RESTAURANT);
            }

            if (Boolean.FALSE.equals(dish.get().getActive())) {
                throw new CustomOrderException(DomainMessagesConstants.ORDER_DISH_NOT_ACTIVE);
            }
